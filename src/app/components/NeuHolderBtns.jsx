...
export default function NeuHolderBtns({ setSongTerm }) {
  ...
  return (
    <div className='sm:ml-[200px] flex flex-row sm:flex-col items-start p-2 sm:space-y-4 align-center justify-center'>
      {Object.entries(buttonDetails).map(([term, { text, icon }]) => ( // Fix: Destructure text & icon properly
        <div 
          key={term} 
        ...
        >
          <div className={`
            ...
            ${expandedTerm === term ? 'w-full' : 'w-11 group-hover:w-full'} 
            ...
            <div className={`
              ...
              ${expandedTerm === term ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} 
